  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar text-xs">
        <span className="flex items-center gap-1 text-slate-400 text-[11px] font-medium flex-shrink-0 mr-1">
        ...
        {samplePrompts.map((prompt, idx) => (
          <button
            ...
            disabled={isLoading}
            className="flex-shrink-0 px-3 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-slate-300 hover:text-slate-100 hover:border-blue-500/40 hover:bg-slate-800 text-[11px] transition-all disabled:opacity-50"
          >
            {prompt}
          </button>
        ))}
      ...
      {/* Input Box */}
      <div className="relative flex items-center rounded-2xl border border-slate-800 bg-slate-900/90 shadow-lg focus-within:border-blue-500/60 focus-within:ring-1 focus-within:ring-blue-500/20 transition-all p-1.5">
        {/* Language selector toggle */}
        <button
          type="button"
          onClick={() => onLanguageChange(language === "en" ? "hi" : "en")}
          className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-800/80 hover:bg-slate-800 text-slate-300 text-xs font-semibold transition-colors flex-shrink-0"
        ...
        <button
          ...
          className="p-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-40 disabled:hover:bg-blue-600 transition-all flex items-center justify-center flex-shrink-0"
          title="Send query"
        >
